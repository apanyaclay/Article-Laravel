                                ...
                                <table id="example1" class="table table-bordered table-striped">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>Author</th>
                                            <th>Views</th>
                                            @canany(['detail article','update article', 'delete article'])
                                            <th>Action</th>
                                            @endcanany
                                        </tr>
                                    </thead>
                                    <tbody>
                                        ...
                                        <tr>
                                            <td>{{$item->id}}</td>
                                            <td>{{$item->title}}</td>
                                            <td>{{is_object($item->category) ? $item->category->name : 'No Category' }}</td>
                                            <td>{!!$item->excerpt!!}</td>
                                            <td>{{$item->status}}</td>
                                            <td>{{is_object($item->Author) ? $item->Author->name : 'No Author'}}</td>
                                            <td>{{$item->views}}</td>
                                            @canany(['detail article','update article', 'delete article'])
                                            <td class="d-flex justify-content-center">
                                                @can('detail article')
                                                <a href="{{route('article.show', ['id' => $item->id])}}" class="btn btn-sm btn-outline-secondary mr-1">Detail</a>
                                                @endcan
                                                @can('update article')
                                                <a href="{{route('article.edit', ['id' => $item->id])}}" class="btn btn-sm btn-outline-warning mr-1">Edit</a>
                                                @endcan
                                                @can('delete article')
                                                <a href="#" data-href="{{route('article.destroy', ['id' => $item->id])}}" onclick="confirmToDelete(this)" class="btn btn-sm btn-outline-danger">Delete</a>
                                                @endcan
                                            </td>
                                            @endcanany
                                        </tr>
